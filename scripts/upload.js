...
export const uploadAll = async (types) => {
  for (const x of types) {
    const response = await uploadContract(x);
    if (response !== "success") {
      return false;
    }
  }
...
if (import.meta.url === url.pathToFileURL(process.argv[1]).href) {
  if (args.length === 0) {
    console.log(
      "No arguments provided, need --contract (snip24, snip721, offspring, factory, all)"
    );
  } else if (args.length === 2 && args[0] === "--contract") {
    const types = ["snip24", "snip721", "offspring", "factory"];
    if (types.includes(args[1])) {
      uploadContract(args[1]).then(() =>
        console.log("New contract details have been saved to config.json")
      );
    } else if (args[1] === "all") {
      uploadAll(types).then(() =>
        console.log("New contract details have been saved to config.json")
      );
    } else {
      console.log(
        "You did not supply a correct argument value! (snip24, snip721, offspring, factory, all)"
      );
    }
  } else {
    console.log(
      "Incorrect arguments provided, need --contract (snip24, snip721, offspring, factory, all)"
    );
  }
}
