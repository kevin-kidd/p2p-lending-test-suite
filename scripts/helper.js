...
export const calculateAmountOwed = (queryLoanResponse) => {
    ...
    const remaining_time = (new Date(ends_at).getTime() / 1000) - current_time_seconds
    const elapsed_time = lock_up_time - remaining_time
    const hours_elapsed = Math.floor(elapsed_time / interest_interval)
    const interest = (principal_asked * rate * hours_elapsed / (lock_up_time / interest_interval)) / Math.pow(10, decimal_places)
