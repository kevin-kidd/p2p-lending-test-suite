...
const setViewingKeys = async (type, updateConfig, contractAddress) => {
  ...
  const key = Math.random().toString(36).substr(4, 10);
...
const initContract = async (type) => {
  ...
  try {
    ...
    const viewing_key = await setViewingKeys(type, false, contractAddress);
...
export const initAll = async (types) => {
  for (const x of types) {
    const response = await initContract(x);
    if (response !== "success") {
      return false;
    }
  }
...
if (import.meta.url === url.pathToFileURL(process.argv[1]).href) {
  if (args.length === 0) {
    ...
    const types = ["snip24", "snip721", "factory", "bond_maker"];
    if (types.includes(args[1])) {
      const viewing_key = await setViewingKeys(
        args[1],
        true,
        config[args[1]].address
      );
      console.log(
        "Set new viewing key: " + viewing_key + " for contract: " + args[1]
      );
    } else {
      console.log(
