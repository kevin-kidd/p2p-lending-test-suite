...
export const getBalances = async () => {
  ...
  try {
    // Fetch SCRT balances
    const lenderBalance = await queries.queryBalance(clients.lender);
    const borrowerBalance = await queries.queryBalance(clients.borrower);
    const taxBalance = await queries.queryBalance(clients.tax);

    // Fetch SNIP-24 balances
    const lenderSnipBalance = await queries.querySnipBalance(clients.lender);
    const borrowerSnipBalance = await queries.querySnipBalance(
      clients.borrower
    );
    ...
    return {
      lender: {
        scrt: trueBalance(lenderBalance.balance.amount),
        snip: trueBalance(lenderSnipBalance.balance.amount),
      },
      borrower: {
        scrt: trueBalance(borrowerBalance.balance.amount),
        snip: trueBalance(borrowerSnipBalance.balance.amount),
      },
      tax: {
        scrt: trueBalance(taxBalance.balance.amount),
...
export const queryActiveLoan = async (offspringAddress) => {
  try {
    const lenderClient = await getClient("lender");

    // Query the listing info from both borrower & client
    const borrowerResponse = await queries.queryListingInfo(
      offspringAddress,
      client
    );
    const lenderResponse = await queries.queryListingInfo(
      offspringAddress,
      lenderClient
    );

    // Validation for active loan
    if (_.isEqual(lenderResponse, borrowerResponse)) {
      if (borrowerResponse.listing_info.status !== "active") {
...
const queryMyOffsprings = async (type) => {
  try {
    ...
    const myOffsprings = (await queries.listMyOffsprings(client))
      .list_my_offspring.active;
...
const queryActiveOffsprings = async () => {
  try {
    const activeOffsprings = (await queries.listActiveOffsprings())
...
const queryInactiveOffsprings = async () => {
  try {
    const inactiveOffsprings = (await queries.listInactiveOffsprings())
