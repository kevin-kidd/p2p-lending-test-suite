...
export const testRepayLoan = async () => {
  try {
    ...
    if (
      new_balances.borrower.snip - trueBalance(requiredPayment) !==
      final_balances.borrower.snip
    ) {
      ...
      console.log(
        "Expected balance: " +
          new_balances.borrower.snip -
          trueBalance(requiredPayment)
      );
    }

    // Verify principal and interest was received by lender
    if (
      final_balances.lender.snip !==
      new_balances.lender.snip +
        trueBalance(owed.interest) +
        trueBalance(owed.principal)
    ) {
    ...
    // Verify tax was sent
    if (
      final_balances.tax.snip - new_balances.tax.snip !==
      trueBalance(owed.fee)
    ) {
    ...
    const result = collateral.every((val) =>
      borrowerTokensResponse.token_list.tokens.includes(val)
    );
